...
#get solar data (API call) (replace with function call) - returns list with data and metadata, data in solar_data$solar_data
...
load(file = "_cache/solar_data.Rdata")  # this is for 4.5 kWp capacity installed   #2022-2024
...
load(file = "_cache/elcons_data.Rdata") # this is for a residential consumption of 3 MWh annually #2022-2024
elcons %>% glimpse()

#get grid distribution costs 
# datetime, date, year, month, day, weekday, hour, grid_cost
load(file = "_cache/grid_cost.Rdata")  # this is STATIC feed-in price for each hour, range 2022-2024
grid_cost_data %>% glimpse()

#get feedin prices (replace with function call)
# datetime, date, year, month, day, weekday, hour, feedin
load(file = "_cache/feed_in_price.Rdata")  # this is STATIC feed-in price for each hour, range 2022-2024
feed_in_data %>% glimpse()

#get electricty prices (replace with function call)
# datetime, date, year, month, day, weekday, hour, price
load(file = "_cache/elprice_static.Rdata")  # this is STATIC price for each hour, range 2022-2024
elprice %>% glimpse()

} else {
  #source the data and call the functions 
  source("read_irradiance_api.R")
  source("residential_electricity_load_profile.R")
  source("electricity_price.R")
  
  #get solar data (API call) - returns list with data and metadata, data in solar_data$solar_data
  # datetime, date, year, month, day, weekday, hour, P  (P is in Watts, change to kWh later)
  # solar data is kept as list, as function myCombineConsAndSolar() handles the conversion to kWh and selection of important cols
  solar_data <- get_solar_data(lat =  system_params$lat  
                               , lon =  system_params$lon
                               , start_date =  system_params$start_date
                               , system_lifetime = system_params$system_lifetime
                               , loss = system_params$lon  # in percent, e.g. 14 means 14 %
                               , angle = system_params$lon # Inclination angle (0° = horizontal)
  ...
  #solar_data$solar_data %>% tail(100) %>% glimpse()
  print(paste0("solar_data$solar_data nrow: ", nrow(solar_data$solar_data), " minyear: ", min(solar_data$solar_data$year), " maxyear: ", max(solar_data$solar_data$year) ) )
  ...
  #elcons %>% glimpse()
  print(paste0("elcons nrow: ", nrow(elcons)))
  ...
  #elprice %>% glimpse()
  print(paste0("elprice nrow: ", nrow(elprice)))
